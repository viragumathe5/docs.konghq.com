...
* `memory`: Metrics about the memory usage.
    * `workers_lua_vms`: An array with all workers of the Kong node, where each
      entry contains worker memory usage information (`http_allocated_gc`) and
      its process identification number (`pid`).
    * `lua_shared_dicts`: An array of information about shared dictionaries,
      where each node contains how much memory is dedicated for the specific
      shared dictionary (`capacity`) and how much of said memory is in use
      (`allocated_slabs`).
