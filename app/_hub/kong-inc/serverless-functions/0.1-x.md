...
So older version would do this (still works with 0.3 and above):
...
with newer version you can return a function to run on each request, allowing
for upvalues to keep state in between requests:
